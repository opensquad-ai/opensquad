...
def show_all_messages(session: dict):
    ...
    for i, msg in enumerate(messages):
        ...
        role_emoji = {"system": "[ASSISTANT]", "user": "[USER]", "assistant": "🤵"}
...
def show_templates(agent_name: str):
    ...
    if base_prompt.exists():
        with open(base_prompt, encoding="utf-8") as f:
            content = f.read()
    ...
    if role_prompt.exists():
        with open(role_prompt, encoding="utf-8") as f:
            content = f.read()
        print(f"\n\n🔷 角色定义 (agents/{agent_name}/role.md)")
        print("-" * 80)
        print(content[:1000] + "..." if len(content) > 1000 else content)
        print(f"\n[LENGTH] 完整长度: {len(content)} 字符")
