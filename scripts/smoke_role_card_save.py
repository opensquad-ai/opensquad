"""Smoke test: launcher saves role cards to writable workspace (not _internal/).

Usage: uv run python scripts/smoke_role_card_save.py
"""
...
def main() -> None:
    ...
    try:
        ...
        expected = os.path.join(APP_DATA, "role_cards", f"{CARD_NAME}.md")
        if not os.path.isfile(expected):
            print(f"FAIL: file not written to workspace: {expected}")
            sys.exit(1)

        internal = os.path.join(os.path.dirname(exe), "_internal", "role_cards", f"{CARD_NAME}.md")
        if os.path.isfile(internal):
            print(f"FAIL: card incorrectly written to read-only bundle: {internal}")
            sys.exit(1)

        print(f"PASS: role card saved to {expected}")
    finally:
        try:
            api(9600, f"/api/role-cards/{CARD_NAME}", method="DELETE")
        except Exception:
            pass
        proc.terminate()
        try:
