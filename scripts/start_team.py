...
def main():
    ...
    for svc_id, svc_info in services.items():
        ...
        if svc_type == "python":
            ...
            if "npm" in shell_cmd and not shutil.which("npm"):
                print("    [WARNING] 'npm' not found in PATH. Skipping frontend start.")
                continue
    ...
    if launcher_enabled:
        ...
        print(
            f"  Launcher will auto-start agents from config: {', '.join(auto_start_list) if auto_start_list else '(all discovered)'}"
        )
    else:
