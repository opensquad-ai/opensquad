...
from opensquad.message_queue import message_queue, parse_websocket_message
...
class ChatProBridge:
    ...
    def login(self) -> bool:
        ...
        try:
            ...
            if r.status_code == 401:
                ...
                reg = requests.post(f"{self.base_url}/api/auth/register", json={
                    "email": self.email,
                    "password": self.password,
                ...
                if reg.status_code in (200, 201):
                    ...
                    logger.info(f"[Bridge] Auto-register 400 (email exists), attempting password reset...")
                    try:
                        reset = requests.post(
            ...
            for g in groups:
                ...
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        self._group_cache[gid] = {
                            "name": detail.get("name", g.get("name", gid)),
                            "members": members
                        }
                    else:
                        self._group_cache[gid] = {
    ...
    async def connect_ws(self):
        ...
        logger.info(f"[Bridge] Connecting WebSocket...")
        ...
        while True:  # Infinite reconnect loop
            try:
                ...
                logger.warning(f"[Bridge] WebSocket disconnected (attempt {_reconnect_attempts}), reconnecting in {round(_delay,1)}s...")
                ...
                if "401" in _msg or "unauthorized" in _msg or "invalid" in _msg and "token" in _msg:
                    ...
                    if self.login():
                        ...
                        logger.error("[Bridge] Re-login failed; will retry connection with old token")
                _delay = min(60, (2 ** min(_reconnect_attempts, 6))) + (uuid.uuid4().int % 1000) / 1000.0
                logger.error(f"[Bridge] WebSocket error: {e} (attempt {_reconnect_attempts}), reconnecting in {round(_delay,1)}s...")
                await asyncio.sleep(_delay)
    ...
    async def _handle_message(self, data: Dict):
        ...
        if msg_type in ["new_message", "message"]:
            ...
            # Put into queue
            from opensquad.message_queue import message_queue, QueueMessage
            ...
            if group_id:
                ...
                try:
                    await bus.emit_async("group_member_update", {
                        "group_id": group_id,
                        "event": msg_type,
                        "data": data.get("data", {}),
                    })
            ...
            if user_id:
                ...
                try:
                    await bus.emit_async("user_status_update", {
                        "user_id": user_id,
                        "status": status,
                        "data": user_data,
                    })
    ...
    def send_message(self, content: str, target_id: str, target_type: str = "group",
                     file_paths: Optional[List[str]] = None, retries: int = 3) -> bool:
        """
        ...
        for attempt in range(retries):
            try:
                ...
                logger.info(f"[Bridge] Sending {target_type} message to {target_id} (attempt {attempt+1}/{retries})...")
                ...
                r.raise_for_status()
                logger.info(f"[Bridge] Message sent successfully")
                return True
