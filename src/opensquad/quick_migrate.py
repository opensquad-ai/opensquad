    ...
    if os.path.exists(old_db):
        ...
        try:
            ...
            if not os.path.exists(new_db):
                ...
                print(f"   [OK] Migrated database: chat.db")
                migrated += 1
            else:
                print(f"   - Skipped (target already exists): chat.db")
    ...
    if os.path.exists(old_sessions):
        ...
        try:
            ...
            if not os.path.exists(new_sessions):
                ...
                print(f"   [OK] Migrated session data: ai_web_sessions.json")
                migrated += 1
            else:
                print(f"   - Skipped (target already exists): ai_web_sessions.json")
        except Exception as e:
