...
def run_config(args):
    ...
    if not os.path.isfile(cfg_path):
        ...
        print("  \u2139 Run 'opensquad init' to create it")
        sys.exit(1)
    ...
    if auth.get("gateway_token") in (None, "YOUR_GATEWAY_TOKEN_HERE", "opensquad-gateway-simple-token", ""):
        ...
        print("  \u26a0 gateway_token: DEFAULT (insecure for production)")
    else:
        print("  \u2705 gateway_token: set")
    ...
    if auth.get("external_api_key") in (None, "YOUR_EXTERNAL_API_KEY_HERE", ""):
        ...
        print("  \u26a0 external_api_key: DEFAULT (insecure for production)")
    else:
        print("  \u2705 external_api_key: set")
