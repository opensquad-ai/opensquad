...
def is_leaked_tool_params(text: str) -> bool:
    ...
    if xml_tags and '<tool_call' not in text:
        ...
        if leaked_tags:
            logger.warning("[Validation] Detected leaked XML parameter tags without <tool_call> wrapper: %s", leaked_tags)
            return True

    return False


def is_repeated_content(text: str, get_messages: Optional[Callable[[], List[Dict[str, Any]]]] = None) -> bool:
    ...
    if get_messages is not None:
        ...
        loop_phrases = [
            "stuck in a repetition loop", "stuck in a loop",
            "apologize for the repetition", "breaking out of the loop",
        ]
