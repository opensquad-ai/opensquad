...
class LegacyDataMigrator:
    ...
    def __init__(self, install_dir: str, target_workspace: str, mode: str = "copy", overwrite: bool = False):
        """
        Args:
    ...
    def migrate(self, progress_callback: Callable[[str], None] | None = None) -> MigrationReport:
        """
        ...
        log(
            f"[Migration] Success: {len(self.report.success)}, Failed: {len(self.report.failed)}, Skipped: {len(self.report.skipped)}"
        )
...
def handle_legacy_migration(install_dir: str) -> str:
    ...
    if choice == "1":
        ...
        if mode == "copy":
            ...
            print("After confirming the workspace is working correctly, you may delete it manually to free disk space.")
