...
async def create_user(db: AsyncSession, user_data: UserCreate, user_id: Optional[str] = None) -> User:
    """Create a new user.
...
def decode_token(token: str) -> Optional[dict]:
    ...
    try:
        ...
        _log.warning(
            "[AUTH] Token decode FAILED: %s: %s (algorithm=%s)",
            type(e).__name__, e, ALGORITHM
        )
        return None
