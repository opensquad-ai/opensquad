...
from app.ai_web.websocket import agent_handler, user_handler, launcher_handler
    ...
    async def _task_reset_users():
        try:
            ...
            async with AsyncSessionLocal() as db:
                await db.execute(
                    update(User).where(User.status == UserStatus.ONLINE).values(status=UserStatus.OFFLINE)
                )
                await db.commit()
...
async def ai_user_chat(websocket: WebSocket, agent_id: str):
    ...
    if cfg_gateway_token and cfg_gateway_token not in ("", "YOUR_GATEWAY_TOKEN_HERE") and hmac.compare_digest(token, cfg_gateway_token):
...
# Packaged: directory containing sys.executable is the PyInstaller dist directory; nexuschat-pro/dist is also there
if _IS_FROZEN:
...
def _is_vite_running(host: str = _VITE_HOST, port: int = _VITE_PORT) -> bool:
    ...
    try:
        with _socket.create_connection((host, port), timeout=0.1):
            return True
    except (OSError, _socket.timeout, TimeoutError):
        return False
...
if _VITE_AVAILABLE:
    _startup_log.info(
        ...
        _VITE_HOST, _VITE_PORT, syscfg.port("gateway"),
    )
...
def _serve_dist_file(path: str):
    ...
    if not file_path.startswith(os.path.normpath(FRONTEND_DIST) + os.sep) and file_path != os.path.normpath(FRONTEND_DIST):
