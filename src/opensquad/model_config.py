...
class ModelConfig:
    ...
    # Prefix-cache opt-in for OpenAI-compatible providers that need a request
    # flag (e.g. DeepSeek chat_template_kwargs.cache.use). OpenAI/Claude cache
    # the stable system prefix automatically; the flag is a no-op there.
    # Default on so DeepSeek (official URL or model name on Ark/compat) hits cache.
    ...
    def from_dict(
        ...
        return cls(
            ...
            prompt_cache=_get("prompt_cache", True),
            # Claude-specific
