...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        self._desc_cache: Optional[str] = None  # cache for generate_tool_descriptions()
    ...
    def register(self, tool_set: object, namespace: str, level: str = "extended"):
        ...
        with self._lock:
            if namespace in self._tools:
                logger.warning(
                    "Tool namespace %r is being re-registered (old=%r, new=%r). "
                    "This may indicate a plugin conflict.",
                    namespace, self._tools[namespace]["module"].__class__.__name__,
    ...
    def generate_openai_tools(self, tool_filter: str = "all") -> List[Dict]:
        ...
        HIGH_FREQ_NAMESPACES = {
            "filesystem", "system", "websearch", "long_memory", "help"
        }
        MEDIUM_FREQ_NAMESPACES = {
            "git", "api_process", "vision", "mcp_query", "im", "media", "translate_tool"
        }
        ...
        with self._lock:
            for namespace, info in self._tools.items():
                ...
                members = list(inspect.getmembers(module, inspect.isfunction)) + \
                         list(inspect.getmembers(module, inspect.ismethod))
    ...
    def _generate_single_tool_desc(self, namespace: str, tool_set: object, detail: bool) -> str:
        ...
        if detail:
            ...
            if doc: lines.append(f"  **Description**: {doc.strip()}")
            ...
            for name, func in inspect.getmembers(tool_set, inspect.isfunction):
                ...
                if id(func) in seen_funcs: continue
                seen_funcs.add(id(func))
                try:
                    sig = inspect.signature(func)
            ...
            lines.append(f"- **`{namespace}`**: {summary}. Contains: {funcs_str} (call `help.get_tool_help(namespace='{namespace}')` for details)")
        return "\n".join(lines)

    def _generate_mcp_desc(self, detail: bool, filter_server: str = None) -> str:
        lines = []
        if detail:
            lines.append("### MCP External Tool Set (detailed)")
            for tool in self._mcp_adapter.get_all_tools():
                f = tool["function"]
                if filter_server and not f['name'].startswith(filter_server): continue
                lines.append(f"\n  **Tool: `{f['name']}`**")
            ...
            server_names = list(self._mcp_adapter._tools_cache.keys()) if hasattr(self._mcp_adapter, '_tools_cache') else []
            servers_str = ", ".join(server_names) if server_names else "None"
            lines.append(f"- **`mcp_tools`**: External MCP service integration. Available servers: {servers_str}. Call `help.get_tool_help(namespace='mcp__<server_name>')` for details")
        return "\n".join(lines)

    async def call(self, tool_name: str, args: Union[str, Dict[str, Any]]) -> Any:
        """
        ...
        if isinstance(args, str):
            ...
            try:
                ...
                tc_log.warning("[registry.call] Invalid JSON args for tool %r: %s", tool_name, args[:200] if args else "")
        ...
        if '.' in tool_name:
            ...
            tc_log.warning("[registry.call] INVALID FORMAT: tool_name=%r has neither '.' nor '__'. Registered namespaces: %s", tool_name, list(self._tools.keys()))
        ...
        try:
            ...
            try:
                    ...
                    if key in hints:
                        ...
                        if origin is list:
                            ...
                            if type_args and type_args[0] is str and isinstance(value, str):
                                ...
                                if ';' in value:
                                    ...
                                    tc_log.debug("[registry.call] Auto-convert %r: str -> List[str] (semicolon, %d items)", key, len(converted_args[key]))
                                else:
                                    # Use comma separator (normal case)
                                    converted_args[key] = [s.strip() for s in value.split(',') if s.strip()]
                                    tc_log.debug("[registry.call] Auto-convert %r: str -> List[str] (comma, %d items)", key, len(converted_args[key]))
                            else:
                        ...
                        # 2. If int expected and a string is passed, auto-convert to integer
                            ...
                            try:
                                ...
                                tc_log.debug("[registry.call] Auto-convert %r: str '%s' -> int %d", key, value, converted_args[key])
                            except ValueError:
                                tc_log.warning("[registry.call] Failed to convert %r='%s' to int, using original value", key, value)
                                converted_args[key] = value
                        ...
                        # 3. If float expected and a string is passed, auto-convert to float
                            ...
                            try:
                                ...
                                tc_log.debug("[registry.call] Auto-convert %r: str '%s' -> float %f", key, value, converted_args[key])
                            except ValueError:
                                tc_log.warning("[registry.call] Failed to convert %r='%s' to float, using original value", key, value)
                                converted_args[key] = value
                        ...
                        # 4. If bool expected and a string is passed, auto-convert to bool
                            ...
                            if value.lower() in ('true', '1', 'yes', 'on'):
                                ...
                                tc_log.warning("[registry.call] Ambiguous bool value %r='%s', defaulting to True", key, value)
                ...
                tc_log.warning("[registry.call] Type conversion failed for %s.%s: %s (using original args)", ns, fn, type_conv_err)
