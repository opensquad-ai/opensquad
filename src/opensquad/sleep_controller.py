...
class SleepController:
    ...
    def __init__(self):
        self._sleep_event: asyncio.Event | None = None  # created lazily
        self._wake_reason: str | None = None  # wakeup reason
        self._start_time: datetime | None = None  # sleep start time
    ...
    async def sleep(
        self, seconds: int, on_wake: Callable | None = None, agent_dir: str | None = None
    ) -> dict[str, Any]:
        """
        Enter sleep state.
        ...
        try:
            ...
            done, pending = await asyncio.wait([event_task], timeout=seconds, return_when=asyncio.FIRST_COMPLETED)
            ...
            if not self._wake_reason:
                self._wake_reason = f"Exception wakeup: {e!s}"
            ...
            logger.info(
                f"[Sleep] Wake up! Type: {wake_type}, Reason: {wake_info['wake_reason']}, Actual: {actual_duration:.1f}s"
            )
            return wake_info
    ...
    def wake_up(self, reason: str) -> bool:
        ...
        if self._is_sleeping and self._sleep_event:
            ...
            return True
        logger.debug("[Sleep] Wake signal ignored (not sleeping)")
        return False
