...
def test_1_crud():
    ...
    # write
    id1 = am.write(topic="AI Development", keywords=["artificial intelligence", "deep learning", "neural networks"],
                    summary="The field of artificial intelligence has advanced rapidly in recent years", source="agent_A")
    id2 = am.write(topic="Quantum Computing", keywords=["quantum", "computing", "qubit"],
                    summary="Quantum computing is expected to break through the limitations of classical computing", source="agent_B")
    id3 = am.write(summary="This is a note without a topic",
    ...
    assert entry1["source"] == "agent_A", f"source mismatch"
    ...
    try:
        ...
        print(f"  [*] write() empty content: ValueError (correct)")
...
def test_3_query(am):
    ...
    am.write(topic="US-China Trade Friction",
    ...
    am.write(topic="Fed Monetary Policy",
    ...
    am.write(topic="A-Share Market Trend",
    ...
    print(f"  [*] fast: {len(r_fast['matched_entries'])} hits, "
    ...
    print(f"  [*] standard: {len(r_std['matched_entries'])} hits, "
    ...
    print(f"  [*] deep: {len(r_deep['matched_entries'])} hits, "
    ...
    print(f"  [*] empty query: 0 hits (correct)")
...
def test_4_chain(am):
    ...
    print(f"  [*] find_chain(['Trump', 'A-shares']):")
...
def test_5_persistence(am):
    ...
    try:
        ...
        print(f"  [*] before save: docs={stats_before['cooccurrence']['total_docs']}, "
        ...
        assert stats_after['cooccurrence']['total_docs'] == \
               stats_before['cooccurrence']['total_docs'], "total_docs mismatch"
        assert stats_after['cooccurrence']['vocab_size'] == \
               stats_before['cooccurrence']['vocab_size'], "vocab_size mismatch"
        assert stats_after['store']['total_entries'] == \
               stats_before['store']['total_entries'], "entries count mismatch"
        assert stats_after['matrices']['ppmi_nnz'] == \
               stats_before['matrices']['ppmi_nnz'], "ppmi_nnz mismatch"
        ...
        print(f"  [*] query after load: {len(r['matched_entries'])} hits, "
              f"{r['search_stats']['time_ms']}ms")

        # Verify writes still work after loading
        new_id = am2.write(topic="Post-load write test", keywords=["test"],
                           summary="written after load", source="test")
        ...
        print(f"  [*] load(non-existent): False (correct)")
    ...
    for me in r_news["matched_entries"]:
        ...
        if entry:
            assert entry.get("source") != "market_agent", \
                f"source_filter failed: included a market_agent entry"
...
def test_7_time_filter():
    ...
    am.write(topic="Long ago", keywords=["history", "past"],
             summary="An event from 30 days ago", timestamp=now - 30 * 86400)
    am.write(topic="Recent event", keywords=["history", "recent"],
             summary="An event from 1 hour ago", timestamp=now - 3600)
    am.write(topic="Just happened", keywords=["history", "current"],
             summary="An event from 5 minutes ago", timestamp=now - 300)
    ...
    assert len(r_10m["matched_entries"]) == 1, \
        f"last 10 minutes should return 1, got {len(r_10m['matched_entries'])}"
    print(f"  [*] time_recent=10min: {len(r_10m['matched_entries'])} entries")

    # Time range
    r_range = am.query(keywords=["history"], depth="fast",
    ...
    print(f"  [*] time_range(2d_ago ~ 5min_ago): "
          f"{len(r_range['matched_entries'])} entries")
...
def test_8_dual_channel():
    ...
    am.ingest_document("Artificial intelligence and machine learning are driving advances in autonomous driving technology")
    ...
    assert docs_after == docs_before, \
        f"add_keywords should not increase total_docs: {docs_before} -> {docs_after}"
    ...
    assert vocab_after > vocab_before, \
        f"new keywords should expand vocab: {vocab_before} -> {vocab_after}"
    ...
    print(f"  [*] new word verification: quantum computing, CRISPR both in vocab")
...
def test_10_log():
    ...
    assert len(am._date_index[today_str]) == 3, \
        f"today should have 3 log entries, got {len(am._date_index[today_str])}"
    ...
    print(f"  [*] category_index: work={len(am._category_index['work'])} entries, "
    ...
    vocab_after = am._cooccurrence.vocab_count
    assert docs_after == docs_before, \
        f"log() should not increase total_docs: {docs_before} -> {docs_after}"
    assert vocab_after > vocab_before, \
        f"log() keywords should expand vocab: {vocab_before} -> {vocab_after}"
    ...
    assert entry1["summary"] == "Completed core module development of the memory system", "content->summary mapping error"
    ...
    try:
        am.log(content="")
        assert False, "empty content should raise ValueError"
    except ValueError:
...
def test_11_recall_by_date(am, today_str):
    ...
    print(f"  [*] field completeness: id={e0['id']}, time={e0['time']}, "
    ...
    print(f"  [*] recall_by_date(1999-01-01): 0 entries (correct)")
...
def test_12_recall_by_range():
    ...
    logs = [
        ...
        ("Team code review", "work", "dev_agent",
         base_dt + datetime.timedelta(hours=3)),
        ("Had ramen for lunch", "life", "life_agent",
         base_dt + datetime.timedelta(hours=4)),
        ("Deployed test server environment", "work", "dev_agent",
         base_dt + datetime.timedelta(days=1, hours=2)),
        ("Watched a movie", "life", "life_agent",
         base_dt + datetime.timedelta(days=1, hours=8)),
        ("Fixed bug on login page", "work", "dev_agent",
         base_dt + datetime.timedelta(days=2, hours=1)),
        ("Ran 5 kilometers", "exercise", "life_agent",
         base_dt + datetime.timedelta(days=2, hours=6)),
        ("Studied Rust programming language", "study", "study_agent",
    ...
    print(f"  [*] full range (02-05~02-08): {r_all['total_count']} entries, "
          f"{len(r_all['days'])} days")

    # Category filter
    r_work = am.recall_by_range("2026-02-05", "2026-02-08", category="work")
    assert r_work["total_count"] == 4, \
        f"work should have 4 entries, got {r_work['total_count']}"
    ...
    assert r_life["total_count"] == 2, \
        f"life should have 2 entries, got {r_life['total_count']}"
    ...
    assert r_dev["total_count"] == 3, \
        f"dev_agent should have 3 entries, got {r_dev['total_count']}"
    ...
    assert r_kw["total_count"] == 1, \
        f"keyword='bug' should return 1 entry, got {r_kw['total_count']}"
    ...
    r_combo = am.recall_by_range("2026-02-05", "2026-02-08",
                                  category="work", source_filter="dev_agent")
    assert r_combo["total_count"] == 3, \
        f"work+dev_agent should have 3 entries, got {r_combo['total_count']}"
    print(f"  [*] category='work' + source='dev_agent': "
          f"{r_combo['total_count']} entries")

    # Partial date range
    r_partial = am.recall_by_range("2026-02-06", "2026-02-07")
    assert r_partial["total_count"] == 4, \
        f"02-06~02-07 should have 4 entries, got {r_partial['total_count']}"
    ...
    print(f"  [*] empty range (2030): 0 entries (correct)")
...
def test_13_summarize():
    ...
    assert result["total_activities"] == 8, \
        f"week should have 8 entries, got {result['total_activities']}"
    ...
    # Verify category statistics
    assert result["by_category"].get("work") == 5, \
        f"work should have 5 entries: {result['by_category']}"
    assert result["by_category"].get("exercise") == 2, \
        f"exercise should have 2 entries: {result['by_category']}"

    # Verify entries count matches total_activities
    assert len(result["entries"]) == result["total_activities"], \
        "entries count should equal total_activities"
    ...
    try:
        ...
        assert False, "year should raise ValueError"
    except ValueError:
...
def test_14_episodic_persistence():
    ...
    try:
        ...
        k_id = am.write(topic="AI Technology", keywords=["deep learning", "Transformer"],
                        summary="Transformer architecture has become the mainstream in NLP", source="kb_agent")

        l_id1 = am.log(content="Debugged a memory system bug",
                       category="work", source="dev_agent",
                       timestamp=base_ts - 600)  # 13:50
        l_id2 = am.log(content="Had pizza for dinner",
                       category="life", source="life_agent",
        ...
        print(f"  [*] before save: knowledge=1, logs=2, "
        ...
        assert am2._date_index == date_index_before, \
            f"date_index mismatch: {am2._date_index} vs {date_index_before}"
        print(f"  [*] date_index consistent: OK")

        # Verify category index consistency
        assert am2._category_index == cat_index_before, \
            f"category_index mismatch"
        print(f"  [*] category_index consistent: OK")

        # Verify episodic_ids consistency
        assert am2._episodic_ids == episodic_ids_before, \
            f"episodic_ids mismatch"
        ...
        assert k_id not in am2._episodic_ids, \
            "knowledge entry should not be in episodic_ids"
        assert l_id1 in am2._episodic_ids and l_id2 in am2._episodic_ids
        print(f"  [*] knowledge/log distinction: OK (k_id not in episodic, l_ids in episodic)")
        ...
        assert stats_after["episodic"]["total_logs"] == \
               stats_before["episodic"]["total_logs"], "total_logs mismatch"
        assert stats_after["episodic"]["total_days"] == \
               stats_before["episodic"]["total_days"], "total_days mismatch"
        print(f"  [*] get_stats episodic consistent: OK")
...
def test_15_log_semantic_query():
    ...
    am.write(topic="Quantum Computing Advances",
    ...
    assert log_id in matched_ids, \
        f"query should find log entry {log_id}, actual hits: {matched_ids}"
    ...
    assert "symposium" in r["prompt_text"], \
        "prompt_text should include log entry content"
    print(f"  [*] prompt_text includes log content: OK")

    # Use standard depth for related expansion query
    r2 = am.query(keywords=["quantum"], depth="standard", token_budget=2000)
    print(f"  [*] standard query: {len(r2['matched_entries'])} entries, "
    ...
    for e in kb_entries:
        assert e["id"] not in am._episodic_ids, \
            f"knowledge entry {e['id']} should not be in episodic_ids"
    print(f"  [*] knowledge/log distinction: OK")
...
def test_16_entry_type_crud():
    ...
    k1 = am.write(topic="Python Basics", keywords=["Python", "programming", "syntax"],
                  summary="Python is a general-purpose programming language", entry_type="knowledge",
                  category="programming", importance=4)
    k2 = am.write(topic="Machine Learning", keywords=["machine learning", "algorithm", "model"],
                  summary="Machine learning is a subfield of AI", entry_type="knowledge",
                  category="AI")

    e1 = am.write(topic="Debugging Experience", keywords=["debugging", "error", "logs"],
                  summary="When encountering a bug, check logs first then reproduce", entry_type="experience",
                  category="development", importance=5)
...
def test_17_access_count():
    ...
    id1 = am.write(topic="Deep Learning Framework", keywords=["deep learning", "PyTorch", "framework"],
                   summary="PyTorch is a popular deep learning framework")
    id2 = am.write(topic="Natural Language Processing", keywords=["NLP", "text", "analysis"],
                   summary="NLP is the AI technology for processing text")
    id3 = am.write(topic="Computer Vision", keywords=["vision", "image", "recognition"],
...
def test_18_importance_ranking():
    ...
    id_low = am.write(topic="Low Priority News", keywords=["economy", "data"],
                      summary="Ordinary economic data report", importance=1, timestamp=old_ts)
    id_high = am.write(topic="Major Economic Event", keywords=["economy", "data"],
                       summary="Major economic policy change", importance=5, timestamp=old_ts)
...
def test_19_reconsolidation():
    ...
    old_id = am.write(topic="Shape of the Earth", keywords=["earth", "shape", "science"],
                      summary="The Earth is spherical", importance=4)
    ...
    new_id = am.write(topic="Earth Shape Correction", keywords=["earth", "shape", "science"],
    ...
    newer_id = am.write(topic="Latest Earth Shape", keywords=["earth", "shape"],
    ...
    weak_id = am.write(topic="Temporary note", keywords=["temporary", "note"],
                       summary="Temporary record", importance=1)
    replace_id = am.write(topic="Formal note", keywords=["formal", "note"],
                          summary="Formal record", importance=3, supersedes=weak_id)
    ...
    safe_id = am.write(topic="Safety test", keywords=["test"],
                       summary="Supersede a non-existent entry", supersedes="nonexist_000")
...
def test_20_consolidate():
    ...
    am2.write(topic="AI Applications", keywords=["artificial intelligence", "applications", "healthcare"],
              summary="AI applications in healthcare")
    am2.write(topic="DL Breakthroughs", keywords=["deep learning", "image", "recognition"],
    ...
    am3.write(topic="Old Knowledge", keywords=["old knowledge", "outdated", "history"],
              summary="Knowledge from a long time ago", timestamp=old_ts)
    am3.write(topic="New Knowledge", keywords=["new knowledge", "latest", "cutting edge"],
              summary="Recently acquired knowledge", timestamp=recent_ts)
