...
def test_1_weighted_short():
    ...
    print(f"  [*] All weights are 1.0 [OK]")
...
def test_2_weighted_long():
    ...
    assert not all(w == 1.0 for w in weights), \
        f"Long-text weights should not all be 1.0: {weights}"

    # Should be sorted descending by weight
    for i in range(len(weights) - 1):
        assert weights[i] >= weights[i + 1], \
            f"Should be sorted descending, pos {i}: {weights[i]} < {weights[i+1]}"

    print(f"  [*] Long-text extracted {len(result)} keywords")
    for word, weight in result[:8]:
        print(f"      {word}: {weight:.4f}")
    print(f"  [*] Weights vary and are descending [OK]")
...
def test_3_query_long_text_mode():
    ...
    am.write(topic="AI history", keywords=["\u4eba\u5de5\u667a\u80fd", "\u56fe\u7075", "\u611f\u77e5\u673a"],
             summary="\u4eba\u5de5\u667a\u80fd\u7684\u5386\u53f2\u53ef\u4ee5\u8ffd\u6eaf\u5230\u56fe\u7075\u6d4b\u8bd5\u548c\u65e9\u671f\u611f\u77e5\u673a\u6a21\u578b",
             importance=4)
    am.write(topic="Deep learning", keywords=["\u6df1\u5ea6\u5b66\u4e60", "\u795e\u7ecf\u7f51\u7edc", "\u53cd\u5411\u4f20\u64ad"],
             summary="\u6df1\u5ea6\u5b66\u4e60\u901a\u8fc7\u591a\u5c42\u795e\u7ecf\u7f51\u7edc\u548c\u53cd\u5411\u4f20\u64ad\u7b97\u6cd5\u5b9e\u73b0\u4e86\u7a81\u7834",
             importance=5)
    am.write(topic="Transformer architecture", keywords=["Transformer", "\u6ce8\u610f\u529b\u673a\u5236", "GPT"],
             summary="Transformer\u67b6\u6784\u57fa\u4e8e\u81ea\u6ce8\u610f\u529b\u673a\u5236\uff0c\u662fGPT\u7b49\u5927\u8bed\u8a00\u6a21\u578b\u7684\u57fa\u7840",
             importance=5)
    am.write(topic="Reinforcement learning", keywords=["\u5f3a\u5316\u5b66\u4e60", "\u5956\u52b1", "\u7b56\u7565"],
             summary="\u5f3a\u5316\u5b66\u4e60\u901a\u8fc7\u5956\u52b1\u4fe1\u53f7\u4f18\u5316\u7b56\u7565\uff0c\u5728\u6e38\u620f\u548c\u673a\u5668\u4eba\u9886\u57df\u8868\u73b0\u4f18\u79c0",
             importance=3)
    am.write(topic="Computer vision", keywords=["\u8ba1\u7b97\u673a\u89c6\u89c9", "\u56fe\u50cf\u8bc6\u522b", "\u5377\u79ef\u795e\u7ecf\u7f51\u7edc"],
             summary="\u8ba1\u7b97\u673a\u89c6\u89c9\u5229\u7528\u5377\u79ef\u795e\u7ecf\u7f51\u7edc\u5728\u56fe\u50cf\u8bc6\u522b\u4efb\u52a1\u4e2d\u53d6\u5f97\u4e86\u7a81\u7834",
             importance=4)
    am.write(topic="Quantum computing", keywords=["\u91cf\u5b50", "\u91cf\u5b50\u6bd4\u7279", "\u9000\u76f8\u5e72"],
    ...
    assert stats["long_text_mode"] == True, \
        f"Long-text mode should be activated, got: {stats.get('long_text_mode')}"
    ...
    print(f"  [*] Long-text mode query passed [OK]")
...
def test_4_query_short_text_mode():
    ...
    am = AgentMemory()
    am.write(topic="Python programming", keywords=["Python", "\u7f16\u7a0b", "\u811a\u672c"],
             summary="Python\u662f\u4e00\u79cd\u901a\u7528\u9ad8\u7ea7\u7f16\u7a0b\u8bed\u8a00")
    am.write(topic="Java programming", keywords=["Java", "\u7f16\u7a0b", "\u9762\u5411\u5bf9\u8c61"],
    ...
    print(f"  [*] Short-text mode compatibility passed [OK]")
...
def test_5_supplement_verification():
    ...
    assert stats["long_text_mode"] == True, "Should trigger long-text mode"
    ...
    if len(result["matched_entries"]) >= 2:
        ...
        print(f"  [*] Supplement keyword verification bonus working [OK]")
    else:
        print(f"  [*] Matched entries: {len(result['matched_entries'])}")
        print(f"  [!] Insufficient entries to verify ranking (but mechanism code confirmed)")

    print(f"  [*] Supplement verification test complete [OK]")
...
def test_6_edge_cases():
    ...
    print(f"  [*] 80 chars=threshold -> short-text mode [OK]")
    ...
    am = AgentMemory()
    am.write(topic="Test", keywords=["\u6d4b\u8bd5", "\u9a8c\u8bc1"], summary="\u8fd9\u662f\u4e00\u4e2a\u6d4b\u8bd5\u6761\u76ee")
    am.rebuild_matrices()
    ...
    print(f"  [*] Edge case tests passed [OK]")
...
def test_7_ai_keywords_merge():
    ...
    am = AgentMemory()
    am.write(topic="Machine translation", keywords=["\u673a\u5668\u7ffb\u8bd1", "\u5e8f\u5217\u5230\u5e8f\u5217", "\u6ce8\u610f\u529b"],
             summary="\u673a\u5668\u7ffb\u8bd1\u4f7f\u7528\u5e8f\u5217\u5230\u5e8f\u5217\u6a21\u578b\u548c\u6ce8\u610f\u529b\u673a\u5236")
    am.write(topic="Dialogue system", keywords=["\u5bf9\u8bdd", "\u804a\u5929\u673a\u5668\u4eba", "\u81ea\u7136\u8bed\u8a00"],
    ...
    print(f"  [*] AI keyword merge test passed [OK]")
