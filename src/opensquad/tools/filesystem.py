...
from opensquad.utils.path_utils import get_workspace_root, is_path_safe as _is_path_safe_unified
    ...
    try:
        ...
        result = {
            ...
            "meta": {
                ...
                "read_range": f"{start_idx+1}-{end_idx}",
                "truncated": truncated
            }
        }
        if truncated:
...
def search_files(path: str = ".", pattern: str = "", include: str = "*",
    ...
    if rg_path:
        return _search_with_ripgrep(rg_path, path, pattern, include,
                                     case_sensitive, context_lines, max_results)
    
    # Fall back to pure Python implementation
    return _search_pure_python(path, pattern, include,
                                case_sensitive, context_lines, max_results)


def _search_with_ripgrep(rg_path: str, path: str, pattern: str, include: str,
...
def _search_pure_python(path: str, pattern: str, include: str,
                         case_sensitive: bool, context_lines: int,
                         max_results: int) -> Dict[str, Any]:
...
def find_files(path: str = ".", pattern: str = "**/*",
               sort_by: str = "name", max_results: int = 200) -> Dict[str, Any]:
    """
