...
async def add_server(server_name: str, command: str, args: list,
                     timeout: int = 30, auto_approve: list = None,
                     env: dict = None) -> str:
    """
    ...
    if adapter is None:
        return json.dumps({
            "status": "error",
            "message": "MCP adapter not initialized. Please enable MCP in config.json first."
        }, ensure_ascii=False)
    ...
    try:
        ...
        return json.dumps({
            "status": "error",
            "message": f"Error adding MCP server: {str(e)}"
        }, ensure_ascii=False)
...
async def remove_server(server_name: str) -> str:
    ...
    try:
        ...
        return json.dumps({
            "status": "error",
            "message": f"Error removing MCP server: {str(e)}"
        }, ensure_ascii=False)
...
async def reload_servers() -> str:
    ...
    try:
        from opensquad.tools.mcp_adapter import _load_mcp_config, DEFAULT_CONFIG_PATH
        new_config = _load_mcp_config(
            adapter.config_path or DEFAULT_CONFIG_PATH,
            adapter.agent_dir
        )
        ...
        return json.dumps({
            "status": "error",
            "message": f"Error reloading servers: {str(e)}"
        }, ensure_ascii=False)
