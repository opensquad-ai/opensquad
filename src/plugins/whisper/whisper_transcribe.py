...
def transcribe_audio_file(audio_path: str, language: str = "zh") -> dict[str, Any]:
    ...
    try:
        ...
        response = requests.post(
            f"{_get_service_url()}/transcribe/url", json={"path": audio_path, "language": language}, timeout=300
        )
        ...
        with open(audio_path, "rb") as f:
            ...
            response = requests.post(f"{_get_service_url()}/transcribe", files=files, data=data, timeout=300)

        if response.status_code == 200:
            result = response.json()
            if result.get("success"):
                return {"success": True, "text": result["text"], "duration": result.get("duration")}
...
def check_whisper_service() -> dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return {"available": False, "error": f"Service returned error status: {response.status_code}"}
    except Exception as e:
        return {"available": False, "error": f"Cannot connect: {e!s}"}
...
TOOLS = {
    "transcribe_audio_file": {
        ...
        "parameters": {
            "audio_path": {"type": "string", "description": "Absolute path to the audio file", "required": True},
            "language": {
                "type": "string",
