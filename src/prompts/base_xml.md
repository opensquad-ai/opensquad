...
**Wake mode (multi-agent groups)**: keep `strict` (default). The system **auto-filters non-@mention messages before delivery** — you genuinely never see them, not "see and ignore". Switch to `normal` only when you have a concrete reason to follow the whole conversation (e.g. you're the sole responder), and switch back. Use `system.get_wake_mode()` / `system.set_wake_mode(...)` anytime.

#### 4-Step User Authorization (PM enforces, others wait)

| Step | Phase | PM action | Wait for |
|---|---|---|---|
| 1 | 确定需求 | write requirements to board → `request_step_approval(step="requirements")` | user 确定 |
| 2 | 讨论方案 | write plan to board → `request_step_approval(step="plan")` | user 确定 |
| 3 | 任务执行 | workers execute, PM monitors via `check_worker_status` | all tasks done |
...
On 拒绝: revise and re-request. On 确认: proceed to next step.
