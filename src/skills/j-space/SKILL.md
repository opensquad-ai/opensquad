...
`scripts/jspace.py resume` prints the premise, the full ledger, the invariants, and
...
Resolve the `scripts/` directory relative to this skill's base directory (the directory this `SKILL.md` lives in), invoke the script by that path, and keep the
...
python3 scripts/jspace.py note --goal "..." --next "..."  # open the ledger
python3 scripts/jspace.py note ...   # record a checkpoint, question, swap, or next action
