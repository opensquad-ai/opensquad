...
def atomic_write_text(path, text):
    ...
    temp_path = None
    try:
        with tempfile.NamedTemporaryFile("w", encoding="utf-8", dir=LEDGER_DIR, prefix=".jspace-", delete=False) as fh:
            temp_path = fh.name
            fh.write(text)
...
def observations(hist):
    ...
    if run[0]["verified"] != run[-1]["verified"] and len({h["next"] for h in run}) == 1:
        found.append("Verified entries are growing but the next action has not changed.")
    return found
...
def mode_note(book, args):
    ...
    if args.core:
        if "—" not in args.core and " - " not in args.core:
            ...
            if idx > len(live):
                ...
                refused.append(("that core entry is already live.", "choose the slot that should actually change"))
            elif idx == len(live):
    ...
    if args.check:
        if not args.by:
            ...
            book["Verified"].append("✓%02d %s — verified by: %s" % (num, args.check, args.by))
            changed = True
            check_recorded = True
    else:
    ...
    if args.close is not None:
        ...
        if idx is None:
            refused.append(("no open question numbered %d." % args.close, "run `seam` to see the list"))
