...
def test_strategy_selection(model_config):
    ...
    print(f"  - 模型支持 Function Calling: {selector._supports_function_calling(model_config.get('provider'), model_config.get('model_name'))}")
...
def test_token_comparison(tools):
    ...
    print(f"  - 节省: ~{xml_token_estimate - native_token_estimate} tokens ({(1 - native_token_estimate/xml_token_estimate)*100:.1f}%)")
    
    print(f"\n注意：")
    print(f"  - XML 模式：工具描述在 System Prompt 中，每次请求都发送")
    print(f"  - Native FC 模式：工具 Schema 通过 'tools' 参数发送，不占用 System Prompt")
    print(f"  - 实际节省取决于 System Prompt 的其他内容长度")
...
def main():
    ...
    try:
        ...
        # 测试 2: 策略选择
        strategy = test_strategy_selection(model_config)
        ...
        # 测试 5: 真实场景准备
        test_queries = test_real_scenario_comparison()
