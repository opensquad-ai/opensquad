...
def test_parse_xml_arguments_simple():
    ...
    assert result == {"to": "test@ai", "content": "hello"}, f"Expected {{'to': 'test@ai', 'content': 'hello'}}, got {result}"
...
def test_streaming_parser_nested_tags():
    ...
    assert len(thought_content) > 0, f"Expected thought content, got empty"
    assert "<tool_call>" in full_thought, f"Expected '<tool_call>' in thought content, got: {full_thought}"
    
    # 验证没有触发 tool_call 处理器
    assert len(tool_call_content) == 0, f"Expected 0 tool_calls (should not trigger inside thought), got {len(tool_call_content)}"
...
def test_streaming_parser_actual_nested_tool_call():
    ...
    assert len(tool_call_content) > 0, f"Expected tool_call content, got empty"
