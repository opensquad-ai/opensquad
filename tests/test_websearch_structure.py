...
def test_websearch_plugin_structure():
    ...
    context = Context(
        ...
        plugin_dir=os.path.join(os.path.dirname(__file__), "..", "plugins", "websearch"),
    )

    # 实例化插件
    plugin = WebSearchPlugin(context)
...
def test_websearch_service_script_exists():
    """测试 main.py 文件存在"""
    project_root = os.path.dirname(os.path.dirname(__file__))
    service_path = os.path.join(project_root, "plugins", "websearch", "service", "main.py")
...
def test_get_tool_modules():
    ...
    test_config = {"port": 9001, "auto_start": False}
    context = Context(
        agent_id="test_agent",
        project_root=os.path.dirname(os.path.dirname(__file__)),
        event_bus=None,
...
if __name__ == "__main__":
    print("开始测试 WebSearch 插件自动启动功能...\n")

    try:
        test_websearch_plugin_structure()
        print()
        test_websearch_plugin_on_load_disabled()
        print()
        test_websearch_service_script_exists()
        print()
        test_get_tool_modules()

        print("\n" + "=" * 50)
        print("所有测试通过！WebSearch 插件服务自动启动功能正常！")
        print("=" * 50)
