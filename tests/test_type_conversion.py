...
async def test_int_conversion():
    ...
    # 测试：传入字符串 "10"
    result = await registry.call("test.tool_with_int", {
        "count": "10",
        "name": "测试"
    })
...
async def test_float_conversion():
    ...
    # 测试：传入字符串 "3.14"
    result = await registry.call("test.tool_with_float", {
        "price": "3.14",
        "currency": "CNY"
    })
...
async def test_bool_conversion_true():
    ...
    for true_value in ["true", "True", "TRUE", "1", "yes", "Yes", "on", "ON"]:
        result = await registry.call("test.tool_with_bool", {
            "enabled": true_value,
            "verbose": "false"
        })
...
async def test_mixed_types():
    ...
    # 测试：所有参数都是字符串，系统自动转换
    result = await registry.call("test.tool_with_mixed", {
        "queries": "查询1,查询2,查询3",
        "max_results": "100",
        "min_score": "0.85",
        "strict": "true"
